...
from .services.feedback import effectiveness_snapshot, record_session_feedback
...
async def ingest_telemetry(payload: TelemetryEventIn, db: Session = Depends(get_db)):
    ...
    agent_recommendation = recommend(actions, session.risk_score, intent, confidence)
...
@app.get("/api/v1/deception-effectiveness")
